...
export class ManagerAgent {
  ...
  constructor() {
    this.agent = new OpenAIAgent({
      model: 'gpt-4o-mini',
      temperature: 0.2,
      system_instruction: `You coordinate a team of specialist agents: greeting, summarizer, time_helper, input_coach.
Decide which agents should respond to the latest user message.
Return JSON matching this schema: {
  "actions": [
    { "agent": "greeting" | "summarizer" | "time_helper" | "input_coach", "instructions"?: string }
  ],
  "notes"?: string
}
Only include agents that materially advance the conversation. Prefer greeting agent for new sessions or topic changes. When the user asks for time conversions include time_helper. Use summarizer for recap requests. Use input_coach to improve phrasing. If no agent is needed return an empty actions array.`,
    });
  }
